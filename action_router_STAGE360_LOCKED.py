# ============================================================
# WHISPLAY ACTION ROUTER — STAGE 360
...
ENGINE_NAME = "ACTION_ROUTER"
ENGINE_VERSION = "360.0-CANDIDATE"
...
STATE = {
    ...
    "hardware_actions_allowed": False,
    "calls": 0,
    "blocked": 0,
    "failures": 0,
    "last_tool": "",
...
def register(
    ...
    permission_level = int(
        metadata.get("permission_level", 1)
    )

    requires_approval = bool(
        metadata.get("requires_approval", True)
    )

    hardware_action = bool(
        metadata.get("hardware_action", False)
    )
...
def call(
    ...
    **kwargs,
    ...
    if tool["permission_level"] > STATE["max_permission_level"]:
        ...
        return {
            "ok": False,
            "tool": name,
            "error": "permission_block",
        }

    if (
        tool["hardware_action"]
        and not STATE["hardware_actions_allowed"]
    ):
        ...
        return {
            "ok": False,
            "tool": name,
            "error": "hardware_block",
        }

    approved = bool(
        kwargs.pop("approved", False)
    )
