# =========================
# WHISPLAY BRAIN CORE — STAGE 120 MERGED
# Offline Conversational Companion Brain
# Core + Personality + Fallback + Context + Growth
# =========================
...
DISPLAY_WORDS = [
    "screen", "display", "pixels", "backlight", "st7789", "ui",
]

AUDIO_WORDS = [
    "audio", "voice", "speaker", "mic", "piper", "vosk", "sound",
]

BRAIN_WORDS = [
    "brain", "personality", "fallback", "cognitive", "router",
...
HOOK_MID = [
    ...
    "that’s where the system gets stable",
...
RESP_DEBUG = [
    "That looks like a file, import, or path issue. We isolate the exact file first, then fix only that point.",
    "That error is useful. It tells us where the system is breaking instead of making us guess.",
    "This is probably not the whole build failing. It is likely one active file calling the wrong layer.",
...
RESP_DISPLAY = [
    "Display issues should stay in the display layer. The brain should only send clean text and state changes.",
...
RESP_BRAIN = [
    ...
    "The brain should decide, shape, and recover — but the core should stay simple.",
...
FALLBACK_PARTIAL = [
    ...
    "Tell me whether this is brain, display, audio, or core.",
...
def learn_pattern(text, intent):
    ...
    if "file" in text.lower():
        patterns["file_focused"] = patterns.get("file_focused", 0) + 1

    if "stage" in text.lower():
        patterns["stage_focused"] = patterns.get("stage_focused", 0) + 1
...
def generate_candidates(text, intent):
    ...
    if FEATURES["conversation_chaining"] and STATE["last_topic"] and intent in ["conversation", "unclear"]:
        candidates.append(f"Still connected to {STATE['last_topic']} — we keep that thread alive and make the next move clean.")
...
def score_response(response, intent):
    ...
    if "internet" in response.lower() and FEATURES["offline_mode"]:
        score -= 4
...
def select_best_response(candidates, intent):
    ...
    return sorted(
        candidates,
        key=lambda r: score_response(r, intent),
        reverse=True
    )[0]
...
def apply_start_hook(response):
    if FEATURES["personality_hooks"] and random.random() < 0.62:
        return f"{pick(HOOK_START)} {response}"
    return response


def apply_mid_hook(response):
    if not FEATURES["mid_hooks"]:
        return response

    words = response.split()

    if random.random() < 0.22 and len(words) > 8:
...
def apply_end_hook(response):
    if FEATURES["end_hooks"] and random.random() < 0.36:
        return f"{response} {pick(HOOK_END)}"
    return response
...
def shape_response(response):
    ...
    response = apply_start_hook(response)
    response = apply_mid_hook(response)
    response = apply_end_hook(response)
...
def think(text):
    ...
    try:
        ...
        response = f"Brain recovered from an internal issue: {e}"
