...
try:
    from brain.brain_cognitive_STAGE420_STATUS_ROUTING_LOCKED import process as brain_process
except Exception as e:
...
# ============================================================
# STAGE 330 — LOCKED SPEECH INPUT PATH
# ============================================================

try:
    from mic_input_adapter_STAGE310_LOCKED import (
        listen_once_with_confidence,
    )
    from speech_interpreter_STAGE420_LOCKED import (
        interpret_speech,
    )
except Exception as e:
    print("[STAGE330 SPEECH IMPORT ERROR]", e)
    listen_once_with_confidence = None
    interpret_speech = None


# ============================================================
# STAGE 340 — LOCKED PERSONALITY OUTPUT PATH
# ============================================================

try:
    from personality_response_STAGE320_LOCKED import apply_style
except Exception as e:
    print("[STAGE340 PERSONALITY IMPORT ERROR]", e)
    apply_style = None


# ============================================================
# STAGE 370 — LOCKED ACTION ROUTER
# ============================================================

try:
    import action_router_STAGE360_LOCKED as action_router
except Exception as e:
    print("[STAGE370 ACTION ROUTER IMPORT ERROR]", e)
    action_router = None


# ============================================================
# STAGE 400 — LOCKED SAFE ACTION SET
# ============================================================

try:
    import safe_actions_STAGE380_LOCKED as safe_actions
except Exception as e:
    print("[STAGE400 SAFE ACTION IMPORT ERROR]", e)
    safe_actions = None


# ============================================================
# STAGE 400 — REGISTER LOCKED STAGE 380 SAFE ACTIONS
# ============================================================

if action_router is not None and safe_actions is not None:
    for action_name in safe_actions.list_safe_actions():
        handler = getattr(safe_actions, action_name, None)
        ...
        action_router.register(
            ...
            metadata={
                ...
                "source": "safe_actions_STAGE380_LOCKED",
                "stage": 380,
            },
        )
...
class SystemController:
    ...
    def get_input(self):
        ...
        try:
            ...
            try:
                if not callable(listen_once_with_confidence):
                    print("[MIC] Stage 330 input unavailable")
                    return ""

                mic_result = listen_once_with_confidence(timeout_s=8.0)

                if not isinstance(mic_result, dict):
                    print("[MIC] invalid Stage 330 mic result")
                    return ""

                heard = (mic_result.get("text") or "").strip()
                confidence = mic_result.get("confidence")
                ...
                if not callable(interpret_speech):
                    print("[MIC] Stage 310 interpreter unavailable")
                    return ""

                # PTT override: no wake word required.
                if force_accept:
                    interpreted = interpret_speech(
                        heard,
                        recognizer_confidence=confidence,
                    )

                    speech = interpreted.get(
                        "speech_interpretation",
                        {},
                    )

                    if not speech.get("accepted", False):
                        print("[MIC] speech confidence rejected")
                        return ""

                    return (
                        speech.get("cleaned_text")
                        or ""
                    ).strip()

                # Wake word gate for blank-enter hands-free mode.
                heard_l = heard.lower()
    ...
    def execute_brain(self, text):
        ...
        try:
            ...
            STATE["mode"] = result.get("mode", "casual")
    ...
    # ========================================================
    # STAGE 370 — ACTION ROUTING
    # ========================================================

    def route_action(self, result):
        if action_router is None:
            return {
                "ok": False,
                "error": "action_router_unavailable",
            }
        ...
        action_name = (
            result.get("action")
            or result.get("meaning", {}).get("action")
        )
        ...
        return action_router.call(
            action_name,
            approved=False,
        )

    # ========================================================
    # STAGE 410 — LIVE ACTION HANDLER
    # ========================================================

    def handle_live_action(self, brain_result):
        ...
        action_result = self.route_action(brain_result)

        if action_result.get("error") != "no_action":
            STATE["last_action_result"] = action_result

        if action_result.get("ok") is True:
            safe_result = action_result.get("result", {})

            if isinstance(safe_result, dict):
                safe_message = safe_result.get("message")

                if safe_message:
                    brain_result["response"] = safe_message
    ...
    def output(self, response):
        ...
        autonomy200_observe('output', response_text)
        ...
        try:
            ...
            if display and hasattr(display, "show_speaking"):
                display.show_speaking(response_text)
    ...
    def run(self):
        while STATE["running"]:
            try:
                ...
                # ----------------------------
                # STAGE 410 — LIVE ACTION ROUTING
                # ----------------------------
                if isinstance(response, dict):
                    self.handle_live_action(response)
